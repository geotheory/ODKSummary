...
//forms = [{"id": "MOAS_OPD_v_9_2", "desc": "MOAS OPD v.9.2"},{"id": "MOAS_Registration_v_9", "desc": "MOAS registration form v.9"}];
//forms = [{"id": "MOAS_OPD_v_9_5", "desc": "MOAS OPD v.9.5"},{"id": "MOAS_Registration_v_9_5", "desc": "MOAS registration form v.9.5"}]
...
function drawGraphs(d){
	...
	var	ageDimension        = ndx.dimension(function(d) {return ''+(5*Math.floor(+d['pat_dt-age_y']/5));}),
		ageCountGroup       = ageDimension.group();

	age_chart
	.width($('#agechart').width())
	.height(Math.max(180, w*.5))
	.x(d3.scale.ordinal().domain( Array.apply(0, Array(15)).map(function(_,b) { return ''+(5 * ((b + 1)-1)); }) ))
	.xUnits(dc.units.ordinal)
	.brushOn(false)
	...
	var morbDimension  = ndx.dimension(function(d) { return d['morb_ties-morb']; }),
		morbCountGroup = morbDimension.group();

	var filtered_group = remove_empty_bins(morbCountGroup);
